...
function displayCountryInfo(countryData, container) {
    ...
    const currencyData = Object.keys(countryData.currencies)[0];
    const currency = countryData.currencies[currencyData];
    ...
    container.innerHTML = `
    <div class="country-name title">${countryData.name.common} ${flagEmoji}</div>
    ...
    document.getElementById('flagContainer').innerHTML = flagUrl ? `<p><strong class="title">Flag:</strong></p><img src="${flagUrl}" alt="${flagAlt}">` : ''
    document.getElementById('coatOfArmsContainer').innerHTML = coatOfArmsUrl ? `<p><strong class="title">Coat of Arms:</strong></p><img src="${coatOfArmsUrl}" alt="Coat of Arms of ${countryData.name.common}">` : '';

    
}

function displayCountryList(countries, container) {
    let listHtml = `<p>Multiple results returned. Please choose one of the countries below:</p><ul>`
